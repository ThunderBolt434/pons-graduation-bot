...
def get_top_holders_pct(token_addr, exclude_addresses=None):
    ...
    detinatori". Foloseste API-ul v2 (REST) al Blockscout, nu vechiul API tip
    module=token, care s-a dovedit dezactivat pe acest explorer. Intoarce None
    daca nu poate fi aflat -- in acel caz, tratam ca 'necunoscut', nu ca 'trece filtrul'."""
    ...
    try:
        ...
        holders_resp = requests.get(
            f"{BLOCKSCOUT_API_V2}/tokens/{token_addr}/holders", timeout=15
        ).json()
        def holder_address(h):
            # Diferite versiuni de Blockscout raporteaza adresa fie direct ca
            # "address_hash", fie ca obiect "address": {"hash": "..."}.
            if isinstance(h.get("address"), dict):
                return h["address"].get("hash", "")
            return h.get("address_hash") or h.get("address") or ""

        real_holders = [
            h for h in holders_resp.get("items", [])
            if holder_address(h).lower() not in exclude
        ]
...
def check_graduations(state):
    ...
    for addr in pending:
        ...
        # Actualizam "ultima verificare" DOAR dupa un apel reusit -- daca a esuat
        # (rate-limit temporar etc.), tokenul ramane in fata cozii si e reincercat
        # la urmatoarea rulare, nu peste multe ore, dupa ce roteste prin toti ceilalti.
        state["check_seq"] += 1
