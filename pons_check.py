...
def scan_new_launches(state):
    ...
    # Diagnostic suplimentar, o singura data: functioneaza eth_getLogs deloc pe acest
    # RPC, pentru orice contract, pe un interval mic recent? Daca da 0 aici, RPC-ul
    # in sine e problema, nu adresa fabricii Pons.
    try:
        baseline = w3.eth.get_logs({"fromBlock": max(latest - 50, 0), "toBlock": latest})
        log.info("DIAGNOSTIC BAZA: %s log-uri TOTALE (orice contract) in ultimele 50 de blocuri", len(baseline))
    except Exception as e:
        log.warning("DIAGNOSTIC BAZA a esuat: %s", e)
